...
const pronounce = () => speak(props.word)
...
onUnmounted(() => {
  ...
  // 移动端收起动画未播完即被卸载（如切换查看其他单词）时，放弃本次延迟 close
  if (closeTimer) {
...
// 定时器 id 必须留存并在卸载时清除：移动端切换查看其他单词时本组件会因 :key
// 变化被立即卸载，若放任 250ms 回调执行 emit('close')，会把刚挂载的新弹窗误关
let closeTimer = null
      ...
      <div class="flex items-start gap-2 mb-2">
        <div class="flex items-baseline gap-2 flex-1 min-w-0">
          ...
          <button
            ...
            @click="pronounce"
            class="self-center flex-shrink-0 p-1 -m-1 text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300"
            title="播放发音"
            aria-label="播放发音"
          >
