...
import { generateArticle, generateArticleMeta, extractTaskFromImages, IMAGE_TOKENS_BUDGET } from '../../services/ai'
...
const taskProgressMax = ref(IMAGE_TOKENS_BUDGET)
...
// 上传 .txt / .md 原文材料（运行时原生 input，绕开 uni-input 对 type=file 的劫持）
async function triggerTxtPicker() {
  const files = await pickFiles({ accept: '.txt,.md' })
  const file = files[0]
  if (!file) return
...
// 拍照 / 上传题目图片 → AI 解析写作要求 → 回填生成参数（多张图片一次请求识别）
async function triggerTaskImagePicker() {
  const files = await pickFiles({ accept: 'image/*', multiple: true })
  await handleTaskImageFiles(files)
}

async function handleTaskImageFiles(files) {
  if (!files.length) return

  taskImageError.value = ''
  taskImagePreviews.value = files.map(f => URL.createObjectURL(f))
  recognizingTask.value = true
  taskProgressMax.value = IMAGE_TOKENS_BUDGET * files.length
  taskProgress.value = 0

  try {
    // 本地预处理所有图片，然后一次请求携带全部图片识别（跨图拼接由模型完成）
    const dataUrls = await Promise.all(files.map(prepareImageForAI))
    const merged = await extractTaskFromImages(dataUrls, (currentTokens) => {
      taskProgress.value = currentTokens
    })
...
async function generate() {
  if (mode.value === 'essay' && essayType.value === 'long') {
    if (!sourceArticle.value.trim()) {
      await alert('读后续写必须提供原文材料（粘贴或上传 .txt）')
      return
    }
  }
  error.value = ''
