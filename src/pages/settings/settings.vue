...
const cacheClearOpen = ref(false)

// 缓存清除完成后：刷新数据统计与内存中的词库缓存
...
async function handleLogout() {
  ...
  try {
    ...
    try {
      ...
      if (!await confirmDialog(`同步失败：${error.message}\n\n继续退出将清除本地数据，未同步的改动会丢失（可取消后先导出备份）。仍要继续吗？`)) return
    }

    // 2. 清除本地数据（含同步快照与归属标记），设置一并重置为默认
    try {
...
// 导入备份：运行时原生 input（模板里的 <input type="file"> 会被编译成 uni-input，
// 其 accept/change/ref 均失效，label 包裹也无法唤起选择器）
async function importAllData() {
  ...
  if (!await confirmDialog('导入将合并现有数据，相同文章和单词会被跳过。确定继续？')) {
    return
  }
  ...
  reader.onload = async (e) => {
    try {
      ...
      if (data.type === 'full_backup' && data.data) {
        ...
        await alert(`导入完成：新增 ${stats.articles} 篇文章，${stats.words} 个单词，${stats.marks} 条标记，跳过 ${stats.skipped} 篇已存在文章`)
      } else {
        await alert('导入完成')
      }
    } catch (error) {
...
<template>
  ...
  <div>
    ...
    <div class="space-y-5">
      ...
      <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-gray-200 dark:border-neutral-800 p-5">
        ...
        <div class="space-y-4">
          ...
          <!-- #ifdef H5 -->
          <!-- 点击单词自动发音开关（浏览器原生 TTS 仅 H5 端可用，安卓 WebView 舍弃） -->
          <div class="border-t border-gray-200 dark:border-neutral-800 pt-4">
            <div class="flex items-start justify-between gap-4">
              <div>
                <h3 class="text-sm font-medium text-gray-700 dark:text-neutral-300">点击单词自动发音</h3>
                <p class="text-xs text-gray-500 dark:text-neutral-400 mt-1">
                  开启后，阅读页点击单词查看释义时自动播放英文读音（使用浏览器内置语音，可在弹窗内点击喇叭重播）。
                </p>
              </div>
              <button
                type="button"
                ...
                :class="[
                  'relative w-11 h-6 rounded-full transition-colors shrink-0',
                  settingsStore.autoPronounce ? 'bg-blue-600' : 'bg-gray-300 dark:bg-neutral-700'
                ]"
              >
                <span
                  :class="[
                    'absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform',
                    settingsStore.autoPronounce ? 'translate-x-5' : ''
                  ]"
                />
              </button>
            </div>
      ...
      <div class="bg-white dark:bg-neutral-900 rounded-lg shadow-sm border border-gray-200 dark:border-neutral-800 p-5">
        <h2 class="text-xl font-semibold text-gray-900 dark:text-neutral-100 mb-4">缓存管理</h2>
        <p class="text-sm text-gray-600 dark:text-neutral-400 mb-4">
          清除后相关内容会在再次使用时重新生成（重新消耗 AI 请求），可按类型和文章范围选择性清除，单词标记与文章内容不受影响
        </p>
        <button
          @click="cacheClearOpen = true"
          class="px-4 py-2 bg-gray-100 dark:bg-neutral-800 text-gray-700 dark:text-neutral-300 rounded-md hover:bg-gray-200 dark:hover:bg-neutral-700 transition-colors font-medium cursor-pointer"
        >
          清除缓存
        </button>
      </div>
    ...
    <CacheClearModal
      :open="cacheClearOpen"
      @close="cacheClearOpen = false"
      @done="handleCacheCleared"
    />
  </div>
  </PageLayout>
</template>
